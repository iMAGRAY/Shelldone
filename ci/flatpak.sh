...
#flatpak run --env=G_DEBUG=fatal-criticals org.freedesktop.appstream-glib validate assets/shelldone.appdata.xml
...
if [ "${CI}" != "yes" ] ; then
  flatpak-builder \
    --state-dir /var/tmp/shelldone-flatpak-builder \
    --install /var/tmp/shelldone-flatpak-repo \
    assets/flatpak/net.shelldone.terminal.json \
    --force-clean --user -y
fi
