...
CATEGORIZE = {
    ...
    r"^shelldone-\d+-\d+-[a-f0-9]+.tar.xz$": "linux_raw_bin",
    r"src.tar.gz$": "src",
    r"^Shelldone-macos-.*.zip$": "macos_zip",
    r"^Shelldone-windows-.*.zip$": "windows_zip",
    r"^Shelldone-.*.setup.exe$": "windows_exe",
...
def categorize(rel):
    ...
    tag_name = "shelldone-%s" % rel["tag_name"]
...
def load_release_info():
    with open("/tmp/shelldone.releases.json") as f:
        release_info = json.load(f)

    with open("/tmp/shelldone.nightly.json") as f:
        nightly = json.load(f)
