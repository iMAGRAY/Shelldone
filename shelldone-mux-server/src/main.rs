...
pub fn spawn_listener() -> anyhow::Result<()> {
    ...
    for unix_dom in &config.unix_domains {
        std::env::set_var("SHELLDONE_UNIX_SOCKET", unix_dom.socket_path());
        let mut listener = shelldone_mux_server_impl::local::LocalListener::with_domain(unix_dom)?;
        thread::spawn(move || {
            listener.run();
        });
    }
