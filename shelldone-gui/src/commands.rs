...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let shelldone_perform_key_assignment_sel = sel!(shelldonePerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    shelldone_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["Shelldone", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "Shelldone" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Shelldone {}", config::shelldone_version()),
                            Some(shelldone_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::shelldone_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit Shelldone".into(),
            doc: "Quits Shelldone".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://shelldone.org/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the shelldone documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/shelldone/shelldone/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit shelldone's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/shelldone/shelldone/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit shelldone's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    vec![
        ...
        OpenUri("https://github.com/shelldone/shelldone/issues/".to_string()),
        ShowDebugOverlay,
        // ----------------- Misc
