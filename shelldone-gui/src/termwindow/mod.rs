...
pub enum TermWindowNotif {
    ...
    GetConfigOverrides(Sender<shelldone_dynamic::Value>),
    SetConfigOverrides(shelldone_dynamic::Value),
...
pub struct TermWindow {
    ...
    pub config_overrides: shelldone_dynamic::Value,
...
impl TermWindow {
        ...
        match self.config.window_close_confirmation {
            ...
            WindowCloseConfirmation::AlwaysPrompt => {
                ...
                let can_close = mux
                    ...
                    .is_some_and(|w| w.can_close_without_prompting());
                if can_close {
                    mux.kill_window(self.mux_window_id);
                    window.close();
                    front_end().forget_known_window(window);
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} shelldone version: {}",
                    render_info,
...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        let mut myself = Self {
            ...
            window_background,
            config: config.clone(),
            config_overrides: shelldone_dynamic::Value::default(),
            palette: None,
            focused: None,
            mux_window_id,
    ...
    fn dispatch_window_event(
        ...
        match event {
            WindowEvent::Destroyed => {
                ...
                // <https://github.com/shelldone/shelldone/issues/3522>
            ...
            WindowEvent::AppearanceChanged(appearance) => {
                ...
                // <https://github.com/shelldone/shelldone/issues/2295>
                config::reload();
        ...
        match notif {
            ...
            TermWindowNotif::PerformAssignment {
                ...
                let result = || -> anyhow::Result<()> {
                    ...
                    // <https://github.com/shelldone/shelldone/issues/3209>
            ...
            TermWindowNotif::MuxNotification(n) => match n {
                ...
                MuxNotification::TabResized(_) => {
                    // Also handled by shelldone-client
                    self.update_title_post_status();
                }
...
impl TermWindow {
    ...
    fn activate_tab(&mut self, tab_idx: isize) -> anyhow::Result<()> {
        ...
        // logic in shelldone/src/main.rs. If you update this, update that!
        let max = window.len();
    ...
    fn show_experience_hub(&mut self) {
        ...
        let workspace_name = window.get_workspace().to_string();
    ...
    fn get_semantic_prompt_zones(&mut self, pane: &Arc<dyn Pane>) -> &[StableRowIndex] {
        ...
        if cache.seqno != seqno {
            ...
            let mut zones: Vec<StableRowIndex> = zones
                .into_iter()
                .filter_map(|zone| {
                    if zone.semantic_type == shelldone_term::SemanticType::Prompt {
                        Some(zone.start_y)
                    } else {
                        None
                    }
            ...
            // <https://github.com/shelldone/shelldone/issues/1121>
            zones.dedup();
            cache.zones = zones;
            cache.seqno = seqno;
        }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            OpenUri(link) => {
                shelldone_open_url::open_url(link);
            }
