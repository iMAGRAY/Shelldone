...
pub fn show_debug_overlay(
    ...
    lua.load("shelldone = require 'shelldone'").exec()?;
    ...
    let version = config::shelldone_version();
    let triple = config::shelldone_target_triple();

    term.render(&[Change::Text(format!(
        "Debug Overlay\r\n\
         shelldone version: {version} {triple}\r\n\
