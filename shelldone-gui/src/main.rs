...
#[derive(Debug, Parser)]
#[command(
    about = "Shelldone Terminal Emulator\nhttps://github.com/shelldone/shelldone",
    version = config::shelldone_version()
)]
struct Opt {
    /// Skip loading shelldone.lua
    #[arg(long, short = 'n')]
    skip_config: bool,
...
enum SubCommand {
    ...
    #[command(name = "connect", about = "Connect to shelldone multiplexer")]
...
async fn async_run_ssh(opts: SshCommand) -> anyhow::Result<()> {
    ...
    if opts.verbose {
        ssh_option.insert("shelldone_ssh_verbose".to_string(), "true".to_string());
    }
...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("SHELLDONE_UNIX_SOCKET", unix_socket_path.clone());
    shelldone_blob_leases::register_storage(Arc::new(
        shelldone_blob_leases::simple_tempdir::SimpleTempDir::new_in(&*config::CACHE_DIR)?,
    ))?;
...
impl PublishPlan {
    pub fn resolve(mux: &Arc<Mux>, config: &ConfigHandle, always_new_process: bool) -> Self {
        ...
        match shelldone_client::discovery::resolve_gui_sock_path(
            &crate::termwindow::get_window_class(),
        ) {
            Ok(path) => Self::UseExistingPath(path),
            Err(_) => Self::PublishOnly,
        }
    }

    pub fn should_publish(&self) -> bool {
        match self {
            Self::UseExistingPath(_) | Self::PublishOnly => true,
    ...
    pub fn try_spawn(
        ...
        if let PublishPlan::UseExistingPath(gui_sock) = &self {
                    ...
                    match res {
                        Ok(res) => {
                            log::info!(
                             ...
                             Use shelldone start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener =
        shelldone_mux_server_impl::local::LocalListener::with_domain(&config::UnixDomain {
    ...
    std::thread::spawn(move || {
        let name_holder;
        if should_publish {
            name_holder = shelldone_client::discovery::publish_gui_sock_path(
                &unix_socket_path,
                &crate::termwindow::get_window_class(),
            );
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running shelldone to do this.
    // We must do this before we start the gui frontend as the scheduler
    // requirements are different.
    let mut publish = PublishPlan::resolve(
        &mux,
        &config,
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("Shelldone panic", s);
        }
        default_hook(info);
    }));
}
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use shelldone_font::parser::ParsedFont;
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = shelldone_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by shelldone because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("net.shelldone.terminal").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["shelldone-gui".to_string()];
    ...
    match sub {
        SubCommand::Start(start) => {
            ...
            shelldone_blob_leases::clear_storage();
            res
        }
