use crate::ICON_DATA;
use anyhow::anyhow;
use config::{configuration, shelldone_version};
...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header(
            "User-Agent",
            &format!("shelldone/shelldone-{}", shelldone_version()),
        )
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/shelldone/shelldone/releases/latest")
}
...
pub fn load_last_release_info_and_set_banner() {
    ...
    if let Ok(data) = std::fs::read(update_file_name) {
        ...
        let current = shelldone_version();
        let force_ui = std::env::var_os("SHELLDONE_ALWAYS_SHOW_UPDATE_UI").is_some();
...
fn set_banner_from_release_info(latest: &Release) {
    ...
    let url = format!("https://shelldone.org/changelog.html#{}", latest.tag_name);
...
fn update_checker() {
    ...
    loop {
        // Figure out which other shelldone-guis are running.
        ...
        let socks = shelldone_client::discovery::discover_gui_socks();
