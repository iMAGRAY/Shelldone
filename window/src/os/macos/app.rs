...
const CLS_NAME: &str = "ShelldoneAppDelegate";
    ...
    unsafe {
        match config::configuration().window_close_confirmation {
            ...
            WindowCloseConfirmation::AlwaysPrompt => {
                ...
                let message_text = nsstring("Terminate Shelldone?");
                let info_text = nsstring("Detach and close all panes and terminate shelldone?");
...
extern "C" fn application_dock_menu(
    ...
    let new_window_item =
        MenuItem::new_with("New Window", Some(sel!(shelldonePerformKeyAssignment:)), "");
    new_window_item
    ...
    Class::get(CLS_NAME).unwrap_or_else(|| {
        ...
        unsafe {
            ...
            cls.add_method(
                ...
                shelldone_perform_key_assignment as extern "C" fn(&mut Object, Sel, *mut Object),
            );
            cls.add_method(
