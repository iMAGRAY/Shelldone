...
impl Window {
    fn create_window(
        ...
        let class = WNDCLASSW {
            ...
            cbWndExtra: 0,
            hInstance: h_inst,
            // FIXME: this resource is specific to the shelldone build and this should
...
unsafe fn update_title_font(hwnd: HWND) {
    ...
    if let Some(lf) = get_title_log_font(hwnd, hdc) {
        *font = shelldone_font::locator::gdi::parse_log_font(&lf, hdc).ok();
    }
                    ...
                    .resolve_dead_key(leader, (modifiers, vk))
                {
                    ...
                    ResolvedDeadKey::InvalidCombination(c) => {
                            ...
                            inner.keyboard_info.is_dead_key_leader(modifiers, vk)
                        {
                            ...
                            // it done.
                            // <https://github.com/shelldone/shelldone/issues/1729>
                            inner.events.dispatch(WindowEvent::KeyEvent(key.clone()));
                            return Some(0);
                        }
