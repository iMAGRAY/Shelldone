...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "SHELLDONE_VERSION".to_string(),
            config::shelldone_version().to_string(),
        );
        env.insert(
            "SHELLDONE_TARGET_TRIPLE".to_string(),
            config::shelldone_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your shelldone configuration
    #[arg(long)]
