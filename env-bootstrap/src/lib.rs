...
pub fn set_shelldone_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("SHELLDONE_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("SHELLDONE_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `Shelldone.AppImage foo`, which is super confusing for everyone!
        ...
        // SHELLDONE_EXECUTABLE_DIR is set by `set_shelldone_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("SHELLDONE_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via shelldone.
        /// However, if we are using the system shelldone to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_shelldone_config_env() {
...
pub fn bootstrap() {
    config::assign_version_info(
        shelldone_version::shelldone_version(),
        shelldone_version::shelldone_target_triple(),
    );
    setup_logger();
    register_panic_hook();

    set_shelldone_executable();
    ...
    // shelldone never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // shelldone or the mux server from inside gnome terminal.
    // <https://github.com/shelldone/shelldone/issues/2237>
