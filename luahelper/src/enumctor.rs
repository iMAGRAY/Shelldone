...
/// The primary usage is to enable `shelldone.action` to have the following
/// behaviors for KeyAssignment:
///
/// `shelldone.action{QuickSelectArgs={}}` -> compatibility with prior versions;
/// the table is passed through and from_dynamic -> lua conversion is attempted.
///
/// `shelldone.action.QuickSelectArgs` -> since the `QuickSelectArgs` variant
/// has a payload that impl Default, this is equivalent to the call above.
///
/// `shelldone.action.QuickSelectArgs{}` -> equivalent to the call above, but
/// explicitly calls the constructor with no parameters.
///
/// `shelldone.action.QuickSelectArgs{alphabet="abc"}` -> configures the alphabet.
///
