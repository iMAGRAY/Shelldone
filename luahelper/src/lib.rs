...
use std::rc::Rc;
use shelldone_dynamic::{FromDynamic, ToDynamic, Value as DynValue};
...
fn lua_value_to_dynamic_impl(
    ...
    Ok(match value {
        ...
        LuaValue::UserData(ud) => match ud.get_metatable() {
            Ok(mt) => {
                if let Ok(to_dynamic) = mt.get::<mlua::Function>("__shelldone_to_dynamic") {
                    match to_dynamic.call(LuaValue::UserData(ud.clone())) {
                        ...
                        Err(err) => {
                            return Err(mlua::Error::FromLuaConversionError {
                                ...
                                to: "shelldone_dynamic::Value",
                                message: Some(format!(
                                    "error calling __shelldone_to_dynamic: {err:#}"
                                )),
                            })
                        }
                    }
...
pub struct ValueLua {
    pub value: shelldone_dynamic::Value,
}
                ...
                if let Ok(mt) = ud.get_metatable() {
                    if let Ok(to_dynamic) = mt.get::<mlua::Function>("__shelldone_to_dynamic") {
                        return match to_dynamic.call(LuaValue::UserData(ud.clone())) {
                            ...
                            .fmt(fmt),
                            Err(err) => write!(fmt, "Error calling __shelldone_to_dynamic: {err}"),
                        };
                    }
                }
