...
# This file hooks up shell integration for shelldone.
# It is suitable for zsh and bash.
#
# Although shelldone is mentioned here, the sequences used are not shelldone
...
#
# SHELLDONE_SHELL_SKIP_ALL - disables all
# SHELLDONE_SHELL_SKIP_SEMANTIC_ZONES - disables zones
# SHELLDONE_SHELL_SKIP_CWD - disables OSC 7 cwd setting
# SHELLDONE_SHELL_SKIP_USER_VARS - disable user vars that capture information
...
if [ "${SHELLDONE_SHELL_SKIP_ALL-}" = "1" ] ; then
  return 0
fi
...
__shelldone_install_bash_prexec() {
...
} # end of __shelldone_install_bash_prexec
...
# command provided by shelldone if shelldone is installed, but falls
# back to a simple printf command otherwise.
__shelldone_osc7() {
  if hash shelldone 2>/dev/null ; then
    shelldone set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed shelldone
...
__shelldone_semantic_precmd_executing=""
__shelldone_semantic_precmd() {
  local ret="$?"
  if [[ "$__shelldone_semantic_precmd_executing" != "0" ]] ; then
    __shelldone_save_ps1="$PS1"
    __shelldone_save_ps2="$PS2"
    ...
    __shelldone_check_ps1="$PS1"
  fi
  if [[ "$__shelldone_semantic_precmd_executing" != "" ]] ; then
  ...
  __shelldone_semantic_precmd_executing=0
}

function __shelldone_semantic_preexec() {
  # Restore the original PS1/PS2 if set
  if [[ -n "${__shelldone_save_ps1+1}" && "${__shelldone_check_ps1-}" == "${PS1}" ]]; then
	  PS1="$__shelldone_save_ps1"
	  PS2="$__shelldone_save_ps2"
	  unset __shelldone_save_ps1
  fi
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __shelldone_semantic_precmd_executing=1
}

__shelldone_user_vars_precmd() {
  __shelldone_set_user_var "SHELLDONE_PROG" ""
  __shelldone_set_user_var "SHELLDONE_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX-}" ]] ; then
    __shelldone_set_user_var "SHELLDONE_IN_TMUX" "1"
  else
    __shelldone_set_user_var "SHELLDONE_IN_TMUX" "0"
  fi

  # You may set SHELLDONE_HOSTNAME to a name you want to use instead
  # of calling out to the hostname executable on every prompt print.
if [[ -z "${SHELLDONE_HOSTNAME}" ]]; then
  if [[ -r /proc/sys/kernel/hostname ]]; then
    __shelldone_set_user_var "SHELLDONE_HOST" "$(cat /proc/sys/kernel/hostname)"
  elif hash hostname 2>/dev/null; then
    __shelldone_set_user_var "SHELLDONE_HOST" "$(hostname)"
  elif hash hostnamectl 2>/dev/null; then
    __shelldone_set_user_var "SHELLDONE_HOST" "$(hostnamectl hostname)"
  else
    __shelldone_set_user_var "SHELLDONE_HOST" "unknown"
  fi
else
  __shelldone_set_user_var "SHELLDONE_HOST" "${SHELLDONE_HOSTNAME}"
fi

}

__shelldone_user_vars_preexec() {
  # Tell shelldone the full command that is being run
  __shelldone_set_user_var "SHELLDONE_PROG" "$1"
}
...
if [[ -z "${SHELLDONE_SHELL_SKIP_SEMANTIC_ZONES-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__shelldone_semantic_precmd
    blehook PREEXEC+=__shelldone_semantic_preexec
  else
    precmd_functions+=(__shelldone_semantic_precmd)
    preexec_functions+=(__shelldone_semantic_preexec)
  fi
fi

if [[ -z "${SHELLDONE_SHELL_SKIP_USER_VARS-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__shelldone_user_vars_precmd
    blehook PREEXEC+=__shelldone_user_vars_preexec
  else
    precmd_functions+=(__shelldone_user_vars_precmd)
    preexec_functions+=(__shelldone_user_vars_preexec)
  fi
fi

if [[ -z "${SHELLDONE_SHELL_SKIP_CWD-}" ]] ; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__shelldone_osc7
  else
    precmd_functions+=(__shelldone_osc7)
  fi
fi
