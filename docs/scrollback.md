Shelldone provides a searchable scrollback buffer with a configurable maximum
...
You can control whether Shelldone displays a scrollbar via your configuration
file:
...
When the search overlay is active the behavior of shelldone changes:
...
to obtain the defaults and extend them. In earlier versions of shelldone there
...
may be more recent than your version of shelldone) is shown below.

You can see the configuration in your version of shelldone by running
`shelldone show-keys --lua --key-table search_mode`.
...
config.keys = {
  -- search for things that look like git hashes
  {
    key = 'H',
    mods = 'SHIFT|CTRL',
    action = shelldone.action.Search { Regex = '[a-f0-9]{6,}' },
  },
}
```
