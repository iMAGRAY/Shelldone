...
Shelldone will automatically set `LANG` appropriately.
...
and Shelldone will automatically set `LANG` appropriately.
...
emoji, or perhaps some kanji) then shelldone will try to locate a fallback
font that does contain that glyph.

Shelldone uses freetype and harfbuzz to perform font shaping and rendering in a
...
that you need in your `.shelldone.lua`:
...
were to emit that same emoji from a script, shelldone would render it correctly.
...
`shelldone` supports [advanced font shaping](config/font-shaping.md), which,
...
experiencing this if, e.g., `!=` becomes rendered as `≠` in `shelldone`.
...
The first thing to note is that `shelldone` will always and only output `UTF-8`
...
section of the docs describes how shelldone processes
...
being set appropriately.  `shelldone` sets this to `xterm-256color` by default,
because shelldone aims to be compatible with with the settings defined by that
...
that shelldone isn't sending the correct input then please [open an
issue](https://github.com/shelldone/shelldone/issues) and include the `xxd` hexdump,
...
Starting in version 20210314-114017-04b7cedd, Shelldone has support for colored
...
You may wish to try these steps to install a copy of a `shelldone` terminfo file;
this will compile a copy of the terminfo and install it into your `~/.terminfo`
directory:

```bash
tempfile=$(mktemp) \
  && curl -o $tempfile https://raw.githubusercontent.com/shelldone/shelldone/master/termwiz/data/shelldone.terminfo \
...
to use either `shelldone ssh` or
...
This is not an issue in Shelldone; the same issue manifests in any terminal
...
* Determine the XCursor theme:
  1. is `xcursor_theme` set in the shelldone configuration?
...
4. If the file exists, then shelldone will try to load it

If no XCursor was found, shelldone will fall back to using the default X11 cursor
...
below, and then moving the mouse over the shelldone window:

```
; SHELLDONE_LOG=window::os::x11::cursor=trace shelldone
...
07:34:40.001  TRACE  window::os::x11::cursor > Using $XDG_DATA_DIRS location "/home/shelldone/.local/share/flatpak/exports/share:/var/lib/flatpak/exports/share:/usr/local/share/:/usr/share/"
07:34:40.001  TRACE  window::os::x11::cursor > icon_path is ["/home/shelldone/.local/share/icons", "/home/shelldone/.icons", "/home/shelldone/.local/share/flatpak/exports/share/icons", "/var/lib/flatpak/exports/share/icons", "/usr/local/share/icons", "/usr/share/icons", "/usr/share/pixmaps", "/home/shelldone/.cursors", "/usr/share/cursors/xorg-x11", "/usr/X11R6/lib/X11/icons"]
07:34:41.838  TRACE  window::os::x11::cursor > candidate for Some(Text) is "/home/shelldone/.local/share/icons/Adwaita/cursors/xterm"
07:34:41.838  TRACE  window::os::x11::cursor > candidate for Some(Text) is "/home/shelldone/.icons/Adwaita/cursors/xterm"
07:34:41.839  TRACE  window::os::x11::cursor > candidate for Some(Text) is "/home/shelldone/.local/share/flatpak/exports/share/icons/Adwaita/cursors/xterm"
...
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.local/share/icons/Adwaita/cursors/top_left_arrow"
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.local/share/icons/Adwaita/cursors/left_ptr"
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.icons/Adwaita/cursors/top_left_arrow"
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.icons/Adwaita/cursors/left_ptr"
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.local/share/flatpak/exports/share/icons/Adwaita/cursors/top_left_arrow"
07:34:42.915  TRACE  window::os::x11::cursor > candidate for Some(Arrow) is "/home/shelldone/.local/share/flatpak/exports/share/icons/Adwaita/cursors/left_ptr"
...
## I'm on macOS and shelldone cannot find things in my PATH

On macOS, shelldone is typically launched directly by the Finder process and inherits
...
However, if you want shelldone to directly spawn some other utility that isn't in that
basic PATH, shelldone will report that it cannot find it.

Probably the easiest to maintain solution is to change something like:

```lua
shelldone.action.SpawnCommandInNewWindow {
  args = { 'nvim', shelldone.config_file },
}
```
...
shelldone.action.SpawnCommandInNewWindow {
  args = {
    os.getenv 'SHELL',
    '-c',
    'nvim ' .. shelldone.shell_quote_arg(shelldone.config_file),
  },
}
```
...
shelldone.action.SpawnCommandInNewWindow {
  args = {
    shelldone.home_dir .. '/.local/bob/nvim-bin/nvim',
...
config.set_environment_variables = {
  ...
  PATH = shelldone.home_dir .. '/.local/bob/nvim-bin:' .. os.getenv 'PATH',
}
```
 ...
 * [shelldone.config_file](config/lua/shelldone/config_file.md)
 * [shelldone.shell_quote_arg](config/lua/shelldone/shell_quote_arg.md)
...
By default, shelldone enables ligature support in the font that you have selected.
