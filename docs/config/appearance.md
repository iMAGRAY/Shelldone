...
Shelldone ships with over 700 color schemes available from
...
[shelldone.gui.get_appearance()](lua/shelldone.gui/get_appearance.md)
...
you need to use [shelldone.color.get_default_colors()](lua/shelldone.color/get_default_colors.md) and explicitly merge them.
...
### Defining a Color Scheme in your `.shelldone.lua`
...
Color schemes names that you define in your `shelldone.lua` take precedence
...
See also [shelldone.get_builtin_color_schemes()](lua/shelldone/get_builtin_color_schemes.md) for
...
the available color schemes for an example](https://github.com/shelldone/shelldone/tree/main/config/src/scheme_data.rs).

It is recommended that you place your custom scheme in a directory
named `$HOME/.config/shelldone/colors` if you're on a POSIX system.

On a Windows system, `shelldone` will search for schemes in a directory
named `colors` that is in the same directory as the `shelldone.exe`.

If you wish to place your color scheme files in some other location, then you
will need to instruct shelldone where to look for your scheme files; the
...
Shelldone supports dynamically changing its color palette via escape sequences.
...
```
  ...
  <video width="80%" controls src="../screenshots/shelldone-dynamic-colors.mp4" loop></video>
...
config.window_frame = {
  ...
  font = shelldone.font { family = 'Roboto', weight = 'Bold' },
...
You can attach an image to the background of the shelldone window:
...
to the directory containing your `shelldone.lua` config file.
...
If your Operating System provides Compositing support then Shelldone is able to
