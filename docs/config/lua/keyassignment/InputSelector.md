...
* `choices` - a lua table consisting of the potential choices. Each entry
  ...
  with [shelldone.format](../shelldone/format.md) to produce styled text.
* `action` - and event callback registered via `shelldone.action_callback`.  The
...
config.keys = {
  {
    ...
    action = act.InputSelector {
      action = shelldone.action_callback(function(window, pane, id, label)
        if not id and not label then
          shelldone.log_info 'cancelled'
        else
          shelldone.log_info('you selected ', id, label)
          pane:send_text(id)
        end
      end),
      title = 'I am title',
      choices = {
        ...
        {
          -- Here we're using shelldone.format to color the text.
          -- You can just use a string directly if you don't want
          -- to control the colors
          label = shelldone.format {
...
config.keys = {
  {
    ...
    action = shelldone.action_callback(function(window, pane)
      ...
      window:perform_action(
        act.InputSelector {
          action = shelldone.action_callback(
            function(inner_window, inner_pane, id, label)
              if not id and not label then
                shelldone.log_info 'cancelled'
              else
                shelldone.log_info('id = ' .. id)
                shelldone.log_info('label = ' .. label)
