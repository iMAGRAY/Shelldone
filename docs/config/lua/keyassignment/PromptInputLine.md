...
* `description` - the text to show at the top of the display area. You may
  embed escape sequences and/or use [shelldone.format](../shelldone/format.md).
...
* `prompt` - the text to show as the prompt. You may embed escape sequences
  and/or use [shelldone.format](../shelldone/format.md).  Defaults to: `"> "`. {{since('nightly', inline=True)}}
...
This example also shows the use of `shelldone.format` to emit colored text.
