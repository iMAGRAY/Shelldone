...
```lua
local tab, pane, window = shelldone.mux.spawn_window {}
```
...
```lua
shelldone.mux.spawn_window { args = { 'top' } }
```
...
```lua
shelldone.mux.spawn_window { cwd = '/tmp' }
```
...
```lua
shelldone.mux.spawn_window { set_environment_variables = { FOO = 'BAR' } }
```
...
```lua
shelldone.mux.spawn_window { domain = { DomainName = 'my.name' } }
```
...
```lua
shelldone.mux.spawn_window { width = 60, height = 30 }
```
...
```lua
shelldone.mux.spawn_window { workspace = { 'coding' } }
```
...
shelldone.mux.spawn_window {
  position = {
    ...
    -- * {Named="HDMI-1"} - uses a screen by name. See shelldone.gui.screens()
    -- origin = "ScreenCoordinateSystem"
  },
}
```

See also [shelldone.gui.screens()](../shelldone.gui/screens.md)
