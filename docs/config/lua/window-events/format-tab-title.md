...
-- or `shelldone cli set-tab-title`, but falls back to the
...
* a table holding `FormatItem`s as used in the [shelldone.format](../shelldone/format.md) function.  This allows formatting style and color information for individual elements within the tab.
...
to [tab_max_width](../config/tab_max_width.md).  Shelldone will then compute
...
`shelldone.on("format-tab-title", ...)` calls.
...
shelldone.on(
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    title = shelldone.truncate_right(title, max_width - 2)
