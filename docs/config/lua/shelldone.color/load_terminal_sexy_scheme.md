# `shelldone.color.load_terminal_sexy_scheme(file_name)`
...
Note that shelldone ships with all of the pre-defined terminal.sexy color
schemes, so this function is primarily useful if you want to design a color
scheme using terminal.sexy and then import it to shelldone.
...
```
> colors, metadata = shelldone.color.load_terminal_sexy_scheme("/path/to/file.json")
> print(colors)
