...
`keys` section in your `~/.shelldone.lua` config file.  For example, you can
...
config.keys = {
  ...
  -- be potentially recognized and handled by the tab
  {
    key = 'm',
    mods = 'CMD',
    action = shelldone.action.DisableDefaultAssignment,
  },
}
```
 ...
 * `LEADER` - a special modal modifier state managed by `shelldone`. See [Leader Key](#leader-key) for more information.
...
If you omit an explicit prefix, shelldone will assume `phys:` and use the
...
In some cases, `shelldone` may not know how to represent a key event in either
...
config.keys = {
  ...
  -- Send "CTRL-A" to the terminal when pressing CTRL-A, CTRL-A
  {
    key = 'a',
    mods = 'LEADER|CTRL',
    action = shelldone.action.SendKey { key = 'a', mods = 'CTRL' },
  },
}
```
