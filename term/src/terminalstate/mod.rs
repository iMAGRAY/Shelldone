...
lazy_static::lazy_static! {
    static ref DB: Database = {
        let data = include_bytes!("../../../termwiz/data/shelldone");
...
impl TerminalState {
    ...
    /// Here in shelldone the terminalstate is isolated from other
    ...
    fn perform_csi_mode(&mut self, mode: Mode) {
        match mode {
            ...
            Mode::SetDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in shelldone's mux
            }
            Mode::ResetDecPrivateMode(DecPrivateMode::Code(
                DecPrivateModeCode::SynchronizedOutput,
            )) => {
            ...
            Mode::QueryDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in shelldone's mux; if we get here, then it isn't enabled,
    ...
    fn checksum_rectangle(&mut self, left: u32, top: u32, right: u32, bottom: u32) -> u16 {
        ...
        */
        ...
        // The concept of uninitialized cells in shelldone is not the same as that on VT520 or that
        ...
        // See: https://github.com/shelldone/shelldone/pull/4565
        if checksum == 0 {
            32u16
        } else {
            checksum
        ...
        match edit {
            ...
            Edit::EraseInLine(erase) => {
                ...
                let range = match erase {
                    ...
                    // <https://github.com/shelldone/shelldone/issues/3548>
