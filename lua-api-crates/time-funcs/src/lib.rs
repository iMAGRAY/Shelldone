...
/// by the user via `shelldone.time.call_after`.
...
const SCHEDULED_EVENTS: &str = "shelldone-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    shelldone_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    shelldone_mod.set("strftime", lua.create_function(strftime)?)?;
    shelldone_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}
