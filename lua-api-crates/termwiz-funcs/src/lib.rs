...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    shelldone_mod.set("nerdfonts", NerdFonts {})?;
    shelldone_mod.set("format", lua.create_function(format)?)?;
    shelldone_mod.set(
        "column_width",
    ...
    shelldone_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    shelldone_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_shelldone_terminfo_renderer();
...
fn permute_mods<'lua>(
    ...
    use shelldone_input_types::Modifiers;
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("Shelldone".into()))
                .term_program_version(Some(config::shelldone_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_shelldone_terminfo_renderer() -> TerminfoRenderer {
