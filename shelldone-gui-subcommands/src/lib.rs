...
pub const DEFAULT_WINDOW_CLASS: &str = "net.shelldone.terminal";
...
pub struct StartCommand {
    ...
    /// in your shelldone configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing shelldone GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of shelldone so that you can wait for the command
    /// to complete by waiting for this shelldone process to finish.
    ...
    /// The default is "net.shelldone.terminal".
    ...
    /// instance of shelldone, including error, update and ssh
    ...
    /// shelldone will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, shelldone will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `shelldone start -- bash -l` will spawn bash
...
pub struct SshCommand {
    ...
    /// `shelldone ssh` is able to parse some (but not all!) options
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `shelldone ssh user@host -- bash -l` will spawn bash
