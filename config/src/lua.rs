...
static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "shelldone-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let sub = shelldone_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            shelldone_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module shelldone.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    shelldone_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// shelldone specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `shelldone` module is registered so that the script can
/// `require "shelldone"` and call into functions provided by
/// shelldone.  The shelldone module contains:
/// * `executable_dir` - the directory containing the shelldone
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the shelldone
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized shelldone).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running shelldone instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `shelldone` module in the script
        let shelldone_mod = get_or_create_module(&lua, "shelldone")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".shelldone"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                shelldone_mod
                    ...
                    .context("set shelldone.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("shelldone_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.shelldone.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        shelldone_mod
            ...
            .context("set shelldone.config_file")?;
        shelldone_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set shelldone.config_dir")?;

        lua.set_named_registry_value("shelldone-watch-paths", Vec::<String>::new())?;
        shelldone_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        shelldone_mod.set("target_triple", crate::shelldone_target_triple())?;
        shelldone_mod.set("version", crate::shelldone_version())?;
        shelldone_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        shelldone_mod.set(
            "running_under_wsl",
        ...
        shelldone_mod.set("font", lua.create_function(font)?)?;
        shelldone_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        shelldone_mod.set("hostname", lua.create_function(hostname)?)?;
        shelldone_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        shelldone_mod.set(
            "has_action",
        ...
        shelldone_mod.set("action_callback", lua.create_function(action_callback)?)?;
        shelldone_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        shelldone_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        shelldone_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        shelldone_mod.set("on", lua.create_function(register_event)?)?;
        shelldone_mod.set("emit", lua.create_async_function(emit_event)?)?;
        shelldone_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        shelldone_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        shelldone_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since shelldone was started
...
fn shell_join_args(_: &Lua, args: Vec<String>) -> mlua::Result<String> {
    shlex::try_join(args.iter().map(|arg| arg.as_ref())).map_err(mlua::Error::external)
}
...
/// <https://github.com/shelldone/shelldone/issues/1736>
/// <https://github.com/shelldone/shelldone/issues/1786>
...
///
/// `shelldone.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `shelldone.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `shelldone.font` documentation.
...
/// This implements `shelldone.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// shelldone.on("event-name", function(arg1, arg2)
///   -- do something
...
const IS_EVENT: &str = "shelldone-is-event-emission";
...
assert(shelldone.emit('foo', 2) == false)

shelldone.on('bar', function (n, str)
...
assert(shelldone.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
