...
pub struct Config {
    ...
    /// When true, launching a new shelldone instance will prefer
    ...
    /// This is off by default because @shelldone and @jsgf find the shift-space
...
impl Default for Config {
    fn default() -> Self {
        ...
        Config::from_dynamic(
            &shelldone_dynamic::Value::Object(Default::default()),
            Default::default(),
        )
        .unwrap()
    }
...
impl Config {
    pub fn load() -> LoadedConfig {
        Self::load_with_overrides(&shelldone_dynamic::Value::default())
    }
    ...
    pub fn load_with_overrides(overrides: &shelldone_dynamic::Value) -> LoadedConfig {
        ...
        let mut paths = vec![PathPossibility::optional(HOME_DIR.join(".shelldone.lua"))];
        for dir in CONFIG_DIRS.iter() {
            paths.push(PathPossibility::optional(dir.join("shelldone.lua")))
        }
        ...
        if cfg!(windows) {
            ...
            if let Ok(exe_name) = std::env::current_exe() {
                if let Some(exe_dir) = exe_name.parent() {
                    paths.insert(0, PathPossibility::optional(exe_dir.join("shelldone.lua")));
                }
            }
        }
        if let Some(path) = std::env::var_os("SHELLDONE_CONFIG_FILE") {
            log::trace!("Note: SHELLDONE_CONFIG_FILE is set in the environment");
        ...
        // We didn't find (or were asked to skip) a shelldone.lua file, so
        // update the environment to make it simpler to understand this
        // state.
        std::env::remove_var("SHELLDONE_CONFIG_FILE");
        std::env::remove_var("SHELLDONE_CONFIG_DIR");
    ...
    pub fn try_default() -> anyhow::Result<LoadedConfig> {
        let (config, warnings) =
            shelldone_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
    ...
    fn try_load(
        ...
        overrides: &shelldone_dynamic::Value,
        ...
        let (config, warnings) =
            shelldone_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
                ...
                std::env::set_var("SHELLDONE_CONFIG_FILE", p);
                if let Some(dir) = p.parent() {
                    std::env::set_var("SHELLDONE_CONFIG_DIR", dir);
                }
                Ok(cfg)
            });
        let cfg = config?;
    ...
    pub(crate) fn apply_overrides_obj_to<'l>(
        ...
        match overrides {
            shelldone_dynamic::Value::Object(obj) => {
    ...
    pub fn compute_extra_defaults(&self, config_path: Option<&Path>) -> Self {
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(shelldone_term::Intensity::Half),
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(false),
            intensity: Some(shelldone_term::Intensity::Bold),
            font: bold,
            ..Default::default()
        });
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(shelldone_term::Intensity::Normal),
            font: italic,
            ..Default::default()
        });
    ...
    pub fn apply_cmd_defaults(
        ...
        // and "$APPDIR" set in the shelldone process. These will be
        // propagated to the child processes. Since some apps (including
        // shelldone) use these variables to detect if they are running in
        ...
        cmd.env("TERM_PROGRAM", "Shelldone");
        cmd.env("TERM_PROGRAM_VERSION", crate::shelldone_version());
    }
}
...
fn default_swap_backspace_and_delete() -> bool {
    // cfg!(target_os = "macos")
    // See: https://github.com/shelldone/shelldone/issues/88
    false
}
...
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    if let Some(runtime) = dirs_next::cache_dir() {
        return Ok(runtime.join("shelldone"));
    }

    Ok(crate::HOME_DIR.join(".local/share/shelldone"))
}
...
/// Behavior when the program spawned by shelldone terminates
#[derive(Debug, FromDynamic, ToDynamic, Clone, Copy, PartialEq, Eq, Default)]
...
impl FromDynamic for BoldBrightening {
    fn from_dynamic(
        value: &shelldone_dynamic::Value,
        options: shelldone_dynamic::FromDynamicOptions,
        ...
        match String::from_dynamic(value, options) {
            Ok(s) => match s.as_str() {
                ...
                s => Err(shelldone_dynamic::Error::Message(format!(
...
pub enum ImePreeditRendering {
    /// IME preedit is rendered by Shelldone itself
    #[default]
    Builtin,
...
/// <https://github.com/shelldone/shelldone/pull/2435>
/// <https://github.com/shelldone/shelldone/issues/2771>
/// <https://github.com/shelldone/shelldone/issues/2630>
