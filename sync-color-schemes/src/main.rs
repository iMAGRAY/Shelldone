...
fn make_cache() -> Cache {
    let file_name = "/tmp/shelldone-sync-color-schemes.sqlite";
...
pub async fn fetch_url(url: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("shelldone-sync-color-schemes/1.0")
        .build()?;
...
",
    ...
    for s in &all {
        if s.data.metadata.shelldone_version.as_deref() == Some("nightly builds only") {
...
impl SchemeSet {
    ...
    pub fn load_existing() -> anyhow::Result<Self> {
        ...
        if let Ok(data) = std::fs::read_to_string(&DATA_FILE_NAME) {
            ...
            struct MetaData {
                name: String,
                aliases: Vec<String>,
                shelldone_version: Option<String>,
            }

            let existing: Vec<Entry> = serde_json::from_str(&data)?;
            for item in existing {
                if let Some(version) = &item.metadata.shelldone_version {
    ...
    pub fn add(&mut self, mut candidate: Scheme) {
        ...
        // Resolve shelldone version information for this scheme
...
async fn main() -> anyhow::Result<()> {
    ...
    schemeses
        .sync_toml("https://github.com/catppuccin/shelldone", "main", "")
        .await?;
    schemeses
    ...
    schemeses
        .sync_toml(
            "https://github.com/Hiroya-W/shelldone-sequoia-theme",
            "main",
            "",
        )
        .await?;
    schemeses
        .sync_toml("https://github.com/dracula/shelldone", "main", "")
        .await?;
    schemeses
        .sync_toml(
    ...
    schemeses
        ...
        .await?;
    schemeses
        .sync_toml("https://codeberg.org/anhsirk0/shelldone-themes", "main", "")
        .await?;
    schemeses
        .sync_toml(
            "https://github.com/hardhackerlabs/theme-shelldone",
            "master",
            "",
        )
        .await?;
    schemeses
        ...
        .await?;
    schemeses
        .sync_toml("https://github.com/eldritch-theme/shelldone", "master", "")
        .await?;
