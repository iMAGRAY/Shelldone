...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(shelldone_dynamic::PlaceDynamic);
    let bounded_where_clause = bound::where_clause_with_bound(&input.generics, bound);

    let tokens = match info.into {
        Some(into) => {
            quote!(
            impl #impl_generics shelldone_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> shelldone_dynamic::Value {
        ...
        None => {
            quote!(
            impl #impl_generics shelldone_dynamic::PlaceDynamic for #ident #ty_generics #bounded_where_clause {
                fn place_dynamic(&self, place: &mut shelldone_dynamic::Object) {
                    #(
                        #placements
                    )*
                }
            ...
            impl #impl_generics shelldone_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> shelldone_dynamic::Value {
                use shelldone_dynamic::PlaceDynamic;

                let mut object = shelldone_dynamic::Object::default();
                self.place_dynamic(&mut object);
                shelldone_dynamic::Value::Object(object)
                }
            }
            )
        }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
    ...
    let tokens = match info.into {
        Some(into) => {
            quote! {
                impl shelldone_dynamic::ToDynamic for #ident {
                    fn to_dynamic(&self) -> shelldone_dynamic::Value {
                        let target : #into = self.into();
                        target.to_dynamic()
                    }
                    ...
                    Fields::Named(fields) => {
                        ...
                        Ok(quote!(
                            Self::#ident { #( #var_fields, )* } => {
                                let mut place = shelldone_dynamic::Object::default();

                                #( #placements )*

                                let mut obj = shelldone_dynamic::Object::default();
