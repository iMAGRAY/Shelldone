...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let placements = placements
        .into_iter()
        .map(|f| f.emit_from_dynamic(&literal))
        .collect::<Vec<_>>();

    let bound = parse_quote!(shelldone_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| shelldone_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        shelldone_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
                    other => Err(shelldone_dynamic::Error::NoConversion {
    ...
    let tokens = quote! {
        impl #impl_generics shelldone_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &shelldone_dynamic::Value, options: shelldone_dynamic::FromDynamicOptions) -> core::result::Result<Self, shelldone_dynamic::Error> {
                use shelldone_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                    ...
                    Fields::Named(fields) => {
                        let var_fields = fields
                            ...
                            .map(|f| {
                                ...
                                info.emit_from_dynamic(&literal)
                            })
                        .collect::<Vec<_>>();
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(shelldone_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| shelldone_dynamic::Error::Message(
            ...
            quote!(
                    match value {
                        Value::String(s) => {
                            match s.as_str() {
                                #( #units )*
                                _ => Err(shelldone_dynamic::Error::InvalidVariantForType {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(shelldone_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        impl shelldone_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &shelldone_dynamic::Value, options: shelldone_dynamic::FromDynamicOptions) -> core::result::Result<Self, shelldone_dynamic::Error> {
                use shelldone_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
