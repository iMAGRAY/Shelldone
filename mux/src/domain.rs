...
impl LocalDomain {
    ...
    async fn fixup_command(&self, cmd: &mut CommandBuilder) -> anyhow::Result<()> {
        if let Some(wsl) = self.resolve_wsl_domain() {
            ...
            *cmd.get_argv_mut() = argv;
            ...
            // Note: SHELLDONE_UNIX_SOCKET, SHELLDONE_CONFIG_(FILE|DIR) and other env
    ...
    async fn build_command(
        ...
        if let Ok(sock) = std::env::var("SHELLDONE_UNIX_SOCKET") {
            cmd.env("SHELLDONE_UNIX_SOCKET", sock);
        }
        cmd.env("SHELLDONE_PANE", pane_id.to_string());
...
impl Domain for LocalDomain {
    ...
    async fn domain_label(&self) -> String {
        if let Some(ed) = self.resolve_exec_domain() {
            match &ed.label {
                Some(ValueOrFunc::Value(shelldone_dynamic::Value::String(s))) => s.to_string(),
